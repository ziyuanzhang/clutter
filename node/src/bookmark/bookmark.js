...
//--------------------------------
const readHtmlFile = (filePath) => fs.readFileSync(filePath, "utf-8");
...
const bookmarksPath = path.join(
  __dirname,
  "./static/original_bookmarks_chrome.html"
);
const bookmarksHtml = readHtmlFile(bookmarksPath);
const bookmarksObj = parse(bookmarksHtml);
fs.writeFileSync(
...
const favoritesPath = path.join(
  __dirname,
  "./static/original_bookmarks_edge.html"
);
const favoritesHtml = readHtmlFile(favoritesPath);
const favoritesObj = parse(favoritesHtml);
fs.writeFileSync(
